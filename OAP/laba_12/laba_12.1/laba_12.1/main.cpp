...
int main()
{
  ...
  for (;;)
  {
    ...
    cout << "8 -  Кол-во листьев(правые дочерние вершины)\n";
    cout << "9 - Смешанный обход \n";
    cout << "10 - Нисходяий обход \n";
    cout << "11 - Проверка сбалансированности \n";
    cout << "12 -  выход\n";
    ...
    case 8:  cout << "Кол-во элементов правого поддерева " << valueElem(Root) << endl; break;
    case 9: LNR(Root); break;
    case 10: LRN(Root); break;
    case 11: balance(Root); break;
    case 12:  exit(0);
    }
  }
}
...
{
  Tree* Prev = NULL;       // Prev - элемент перед текущим DOBAVUL NULL
...
int height(Tree* t) {//вспомогательная функция
  if (t == NULL)
    return 0;
  int a = height(t->Left);
...
bool isBalace(Tree* t) {
  if (t == NULL)
    return true;
  return (abs(height(t->Left) - height(t->Right)) <= 1) && isBalace(t->Left) && isBalace(t->Right);
